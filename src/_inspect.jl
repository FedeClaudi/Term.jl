...
"""
function style_methods(methods::Union{Vector{Base.Method}, Base.MethodList}, tohighlight::AbstractString)
    mets = []
    prevmod = ""
    for (i, m) in enumerate(methods)
        code = split(split(string(m), " in ")[1], string(m.name))[2] |> highlight_syntax
        code = "{dim}" * code * "{/dim}"
        code = replace(code, tohighlight => "{$pink_light default}$tohighlight{/$pink_light default}{dim}") 
        code = RenderableText("     {$pink dim}($i){/$pink dim}  {$fn_col}$(m.name){/$fn_col}" * code)
        info = string(m.module) != prevmod ? RenderableText("{bright_blue}   ────── Methods in {$pink underline bold}$(m.module){/$pink underline bold} for {$pink}$tohighlight{/$pink} ──────{/bright_blue}") : nothing
