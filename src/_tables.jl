...
if !isnothing(header) && show_header
    length(header) != N_cols && push!(problems, "Got header with length $(length(header)), expected $N_cols")
    (!isa(header_style, String) && length(header_style) != N_cols) && push!(problems, "Got header_style with length $(length(header_style)), expected $N_cols")
    (!isa(header_justify, Symbol) && length(header_justify) != N_cols) && push!(problems, "Got header_justify with length $(length(header_justify)), expected $N_cols")
end

# check columns
(!isa(columns_style, String) && length(columns_style) != N_cols) && push!(problems, "Got columns_style with length $(length(columns_style)), expected $N_cols")
(!isa(columns_justify, Symbol) && length(columns_justify) != N_cols) && push!(problems, "Got columns_justify with length $(length(columns_justify)), expected $N_cols")


# check footer
if !isnothing(footer)
    isa(footer, Function) || (length(footer)) != N_cols && push!(problems, "Got footer with length $(length(footer)), expected $N_cols")
    (!isa(footer_style, String) && length(footer_style) != N_cols) && push!(problems, "Got footer_style with length $(length(footer_style)), expected $N_cols")
    (!isa(footer_justify, Symbol) && length(footer_justify) != N_cols) && push!(problems, "Got footer_justify with length $(length(footer_justify)), expected $N_cols")
end

# check hpad
(!isa(hpad, Int) && length(hpad) != N_cols) && push!(problems, "Got hpad with length $(length(hpad)), expected $N_cols")
(!isa(vpad, Int) && length(vpad) != N_rows) && push!(problems, "Got vpad with length $(length(vpad)), expected $N_rows")
