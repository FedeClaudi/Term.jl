...
function table_row(cells, widths, box, top_level, mid_level, bottom_level, box_style)
    ...
    l, m, r = apply_style(l, box_style),
    apply_style(m, box_style),
    apply_style(r, box_style)

    # pad cell elements to correct width
    cells = map(c -> pad(c[2], widths[c[1]], :center), enumerate(cells))

    # create row
    if length(widths) > 1
        mid = l * join(cells[1:(end - 1)], m) * m * cells[end] * r
    else
        mid = l * cells[1] * r
    end
...
function make_cells_entries(
    ...
    cells = map(i -> apply_style(pad(entries[i], widths[i], justify[i]), style[i]), 1:N)
    return cells
end
...
function assert_table_arguments(
    ...
    if !isnothing(header)
        length(header) != N &&
            push!(problems, "Got header with length $(length(header)), expected $N")
        (!isa(header_style, String) && length(header_style) != N) && push!(
            problems,
            "Got header_style with length $(length(header_style)), expected $N",
        )
        (!isa(header_justify, Symbol) && length(header_justify) != N) && push!(
            problems,
            "Got header_justify with length $(length(header_justify)), expected $N",
        )
    end

    # check columns
    (!isa(columns_style, String) && length(columns_style) != N) && push!(
        problems, "Got columns_style with length $(length(columns_style)), expected $N"
    )
    (!isa(columns_justify, Symbol) && length(columns_justify) != N) && push!(
        problems,
        "Got columns_justify with length $(length(columns_justify)), expected $N",
    )
    ...
    if !isnothing(footer)
        isa(footer, Function) ||
            (length(footer)) != N &&
                push!(problems, "Got footer with length $(length(footer)), expected $N")
        (!isa(footer_style, String) && length(footer_style) != N) && push!(
            problems,
            "Got footer_style with length $(length(footer_style)), expected $N",
        )
        (!isa(footer_justify, Symbol) && length(footer_justify) != N) && push!(
            problems,
            "Got footer_justify with length $(length(footer_justify)), expected $N",
        )
    end
...
function Table(
    ...
    rows_values = collect(map(r -> collect(map(c -> string.(r[c]), sch.names)), rows))
    ...
    for (l, row) in enumerate(rows_values)
        ...
        if l == 1
            ...
            push!(lines, table_row(row, widths, box, nothing, :mid, bottom, style))
        elseif l == nrows
            push!(
                lines,
                table_row(
            ...
            push!(lines, table_row(row, widths, box, nothing, :mid, :row, style))
        end
    end
