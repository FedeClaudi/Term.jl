...
function Base.show(io::IO, ::MIME"text/plain", theme::Theme)
    ...
    fields = map(
            v -> v[2] isa String ? RenderableText(string(v[1]); style=v[2]) : RenderableText(string(v[1])),
            zip(fields, values),
        )
    ...
    content =  hLine(30, "Base"; style="#9bb3e0") / (rvstack(values[2:13]) * Spacer(2, 1) * lvstack(fields[2:13]))
