...
parse_md(italic::Markdown.Italic; kwargs...)::String =
    join(map(ln -> "{italic}$(parse_md(soft_breaks(ln); inline = true, kwargs...)){/italic}", italic.text))

parse_md(bold::Markdown.Bold; kwargs...)::String =
    join(map(ln -> "{bold}$(parse_md(soft_breaks(ln); inline = true, kwargs...)){/bold}", bold.text))
