...
function split_tags_into_words(text)
    ...
    for markup in tags
        ...
        if length(tag_words) > 1
            ...
            try
                text = text[1:tag.offset-1] * 
                        openers  * 
                        text[tag.offset+textwidth(markup)+2:close_tag.offset-1] * 
                        closers * 
                        text[close_tag.offset+textwidth(markup)+3:end]
            catch
                text = text[1:prevind(text, tag.offset-1)] * 
                    openers  * 
                    text[nextind(text, tag.offset+textwidth(markup)+2):prevind(text, close_tag.offset-1)] * 
                    closers * 
