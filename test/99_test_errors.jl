...
@testset "ERROR messages" begin
    ...
    @test error_message(StackOverflowError()) == "Stack overflow error: too many function calls."

    @test error_message(UndefKeywordError(:x)) == "Undefined function keyword argument: `x`."
    @test error_message(KeyError(:test)) == "Key `test` not found!"
    @test error_message( UndefVarError(:x)) == "Undefined variable `x`."
    @test error_message( UndefVarError(:x)) == "Undefined variable `x`."
    @test  error_message(StringIndexError("test", 21)) == "attempted to access a String at index 21\n"

    @test error_message(InexactError(:round, Int, 1.5)) == "Cannot convert 1.5 to type ::Int64\nConversion error in function: round"
    @test error_message(LoadError("test.jl", 21, ErrorException("msg"))) == "At {grey62 underline}test.jl{/grey62 underline} line {bold}21{/bold}\nThe cause is an error of type: {red}ErrorException"
    @test error_message( BoundsError([1, 2], 3)) == "Attempted to access a {#FFEE58}`Vector{{Int64}}`{/#FFEE58} width shape ({#90CAF9}2{/#90CAF9},) at index {#90CAF9}3{/#90CAF9}\n"
    @test error_message( DomainError(-1, "test")) == "test\nThe invalid value is: -1."
    @test error_message(TypeError(:Panel, "aa", Int, Float64)) == "In `Panel` > `aa` got {#FFF59D bold}Float64{/#FFF59D bold}(::DataType) but expected argument of type ::Int64"

    @test error_message(MethodError(:print, (Panel, Int))) == "(\"objects of type \", Symbol, \" are not callable\")\n \n{dim}No alternative candidates found"
    @tet error_message(MethodError(print, (Panel, Int))) == "No method matching {bold #42A5F5  bold}`print`{/bold #42A5F5  bold} with arguments types:\n{#CE93D8}::DataType{/#CE93D8}, {#CE93D8}::DataType{/#CE93D8}\n                       \nAlternative candidates:\n  \e[38;2;242;215;119mprint\e[39m(::Any...)      \n  \e[38;2;242;215;119mprint\e[39m(\e[31m::IO\e[39m, ::Any)   "
