...
@testset "\e[34mPanel + renderables" begin
    ...
    testpanel(
        Panel(RenderableText("x" .^ 500); fit = true), displaysize(stdout)[2], nothing
    )
end
...
@testset "\e[34mTBOX" begin
    ...
    for justify in (:left, :center, :right)
        ...
        testpanel(
            TextBox("truncate"^25; width = 100, fit = :truncate, justify = justify), 100, 3
        )
        ...
        testpanel(
            TextBox("{red}truncate{/red}test"^8; fit = :fit, justify = justify), 102, 3
        )

        testpanel(
            TextBox("{red}tru\nncate{/red}test"^1; fit = :fit, justify = justify), 15, 4
        )
    end
end
