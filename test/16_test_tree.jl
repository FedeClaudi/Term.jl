...
[1, 2, [2, 3, 4]],
[1, [2, [3, 4], "a"^200], :c, OrderedDict(2=>"a", 1=>:ok, "a"=>2, :test=>[1, 2])],
...
@testset "\e[34mTree" begin
    ...
    for (i, theme) in enumerate((thm1, thm2))
        for (j, guides_type) in enumerate((:standardtree, :boldtree, :asciitree))
            for (k, tree) in enumerate(trees)
                IS_WIN || @compare_to_string string(Tree(tree; theme=theme, guides=guides_type, printkeys=true)) "tree_$(i)_$(j)_$(k)"
            end
        end
    end
    ...
    # test printing
    @test sprint(io -> show(io, Tree(trees[1]))) == "\e[38;5;117mTree <: AbstractRenderable\e[0m \e[2m(h:10, w:80)\e[0m"
